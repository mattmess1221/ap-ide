...
public class GenerateEclipseApt extends AptTask {
    ...
    private void createAptPrefs() {
        ...
        try {
            ...
            defaults.load(new StringReader(ECLIPSE_APT_PREFS_CONTENTS));
            Properties props = new Properties(defaults);
            if (file.exists()) {
                props.load(new FileReader(file));
            } else {
                props.load(new StringReader(ECLIPSE_APT_PREFS_CONTENTS));
                file.createNewFile();
            }
            props.setProperty("org.eclipse.jdt.apt.aptEnabled", "true");
            props.store(new FileWriter(file), "Generated by Kappa-gradle");
        } catch (IOException e) {
            throwError(e);
        }
    }
